...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return program ? { title: program.title, description: program.shortDescription, ...(program.coverImage ? { openGraph: { images: [getPublicImageUrl(program.coverImage)] } } : {}) } : { title: "البرنامج غير موجود" };
}
...
export default async function ProgramDetails({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  return <><section className={`details-hero ${program.coverImage ? "" : "no-image"}`}>
    {program.coverImage && <Image src={getPublicImageUrl(program.coverImage)} alt={`غلاف ${program.title}`} fill priority sizes="100vw"/>}
