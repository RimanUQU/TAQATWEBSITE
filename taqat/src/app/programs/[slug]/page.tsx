...
export default async function ProgramDetails({
  ...
  const registered =
    user &&
    program.registrations.some((item) => item.userId === user.id && item.status === "CONFIRMED");
  const count = program.registrations.filter((item) => item.status === "CONFIRMED").length,
    closed = program.registrationDeadline < new Date(),
    full = count >= program.capacity;
  const errors: Record<string, string> = {
    closed: "انتهت فترة التسجيل في هذا البرنامج.",
    full: "اكتمل عدد المقاعد المتاحة.",
    duplicate: "أنتِ مسجلة بالفعل في هذا البرنامج.",
  };
  return (
    <>
        ...
        <div className="container two-col">
          <div>
            ...
            <div className="details-list">
              ...
              <div className="detail-box">
                <small>المدة</small>
                <strong>{differenceInDays(program.endDate, program.startDate) + 1} أيام</strong>
              </div>
              <div className="detail-box">
                <small>الموقع</small>
          ...
          <Card className="register-box">
            <h2>جاهزة للانضمام؟</h2>
            <p>احجزي مقعدك وابدئي تجربة جديدة مع مجتمع طاقات.</p>
            {state.success && <Alert type="success">تم تسجيلك في البرنامج بنجاح.</Alert>}
            {state.error && (
              <Alert type="error">{errors[state.error] || "تعذّر إتمام التسجيل."}</Alert>
            )}
            {registered ? (
              <Alert type="success">أنتِ مسجلة في هذا البرنامج.</Alert>
            ) : (
              <form action={registerProgramAction.bind(null, program.id)}>
                <Button type="submit" disabled={closed || full}>
                  {closed ? "انتهى التسجيل" : full ? "اكتملت المقاعد" : "التسجيل في البرنامج"}
                </Button>
              </form>
            )}
